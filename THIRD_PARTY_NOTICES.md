# Stemcraft third-party notices

Stemcraft includes the following open-source software:

- **demucs-rs** — Copyright its contributors; Apache License 2.0.
  <https://github.com/nikhilunni/demucs-rs>
- **FFmpeg 8.0.1** — Copyright the FFmpeg developers; GNU Lesser General
  Public License 2.1 or later. Stemcraft's release build disables GPL and
  nonfree components. <https://ffmpeg.org/>
- **LAME 3.100** — Copyright its contributors; GNU Lesser General Public
  License 2.0. <https://lame.sourceforge.io/>

The `htdemucs_ft` model weights are not included in the app. Stemcraft offers
to download them from the `set-soft/audio_separation` repository on first use.
The repository identifies these weights as MIT licensed.
<https://huggingface.co/set-soft/audio_separation>

Full license texts for bundled components are included in the app's
`Contents/Resources/Licenses` directory.
